...
$stmt  = $jadwal->runQuery("SELECT no_ktp FROM tb_info_interview WHERE :id");
$stmt->execute(array(
  ':id' => $id));
  if ($stmt->rowCount() == 0) {
...
if (isset($_POST['addJadwal'])) {
  ...
  if (!$stmt) {
    ...
    print "<script>window.location='index.php?p=schedule-interview';</script>";
  }
...
if (isset($_POST['addJadwal'])) {
  ...
  $input = new Karyawan();
  $stmt = $input->runQuery("UPDATE tb_info_interview SET kd_interview = :kode, date_interview = :tanggal, detail = :detail, kd_admin = :admin WHERE no_ktp = :id");
  $stmt->execute(array(
    ':kode'   => $kodeTest,
    ':tanggal'=> $tanggalTest,
    ':detail' => $detail,
